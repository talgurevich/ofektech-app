...
export async function POST(request: Request) {
  ...
  const roleHebrew =
    role === "mentor" ? "מנטור/ית" : role === "admin" ? "מנהל/ת" : "יזם/ית";

  const loginUrl = `${request.headers.get("origin") || "https://ofektech-app.vercel.app"}/login`;

  const { error: emailError } = await resend.emails.send({
    from: "OfekTech <onboarding@resend.dev>",
    to: email,
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <ul style="margin:0 0 28px;padding:0 20px 0 0;font-size:15px;color:#555;line-height:2;">
                ${
                  role === "mentor"
                    ? `<li>לתעד מפגשי מנטורינג ולשתף תובנות</li>
                <li>לעקוב אחרי ההתקדמות של היזמים שלך</li>
                <li>לתת ולקבל משוב על מפגשים</li>
                <li>לצפות בלוח הזמנים של התוכנית</li>`
                    : `<li>למלא משוב שבועי ולעקוב אחרי ההתקדמות</li>
                <li>לדרג הרצאות ומפגשי מנטורינג</li>
                <li>לראות את לוח הזמנים והמפגשים הקרובים</li>
                <li>להגדיר יעדים שבועיים ולבדוק את ההישגים</li>`
                }
              </ul>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${loginUrl}" style="display:inline-block;background-color:#22c55e;color:#ffffff;font-size:16px;font-weight:bold;text-decoration:none;padding:14px 40px;border-radius:10px;">
                      כניסה לפורטל
                    </a>
                  </td>
                </tr>
